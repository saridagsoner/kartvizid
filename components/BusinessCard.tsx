...
const BusinessCard: React.FC<BusinessCardProps> = ({ cv, onClick }) => {
  return (
    <div
      onClick={onClick}
      className="bg-white dark:bg-gray-800 rounded-[2.5rem] p-6 border border-gray-200 dark:border-gray-700 card-hover cursor-pointer relative animate-fade-in shadow-sm"
    >
      <div className="flex flex-col sm:flex-row items-center sm:items-start gap-8">
        {/* Photo Section - Adjusted height and added thin frame */}
        <div className="relative shrink-0">
          <div className="w-24 h-28 rounded-[1.75rem] border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm group bg-gray-50 dark:bg-gray-700">
            <img
              src={cv.photoUrl}
              alt={cv.name}
        ...
        <div className="flex-1 text-center sm:text-left pt-1">
          ...
          <p className="text-base text-gray-500 dark:text-gray-400 font-bold mb-4 tracking-tight">{cv.profession}</p>

          {/* Details Section with Simple Icons */}
          <div className="flex flex-wrap justify-center sm:justify-start gap-x-6 gap-y-3 text-[13px] text-gray-500 dark:text-gray-400 font-bold">
          ...
          {/* Summary Section */}
          <div className="mt-6">
            <p className="text-[13px] text-gray-600 dark:text-gray-300 font-medium leading-relaxed line-clamp-2 italic">
              "{cv.about}"
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center mt-4 sm:mt-0 sm:self-center">
          <button className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 text-black dark:text-white px-8 py-3 rounded-full font-black text-xs hover:bg-[#1f6d78] hover:text-white hover:border-[#1f6d78] dark:hover:border-[#1f6d78] transition-all active:scale-95 shadow-sm uppercase tracking-widest shrink-0">
            Görüntüle
          </button>
        </div>
      </div>
