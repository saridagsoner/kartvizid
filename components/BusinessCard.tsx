...
const BusinessCard: React.FC<BusinessCardProps> = ({ cv, onClick, isActive }) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 right-4 sm:right-10 left-[82px] sm:left-[92px] border-b border-gray-200 dark:border-white/10" />

      <div className="flex items-start gap-4 sm:gap-7">
        {/* Photo Section - Custom mobile size */}
        <div className="relative shrink-0 flex items-center">
          <div className="w-[58px] h-[70px] sm:w-[60px] sm:h-[72px] rounded-lg sm:rounded-xl overflow-hidden bg-gray-50 dark:bg-black shadow-sm border border-gray-100 dark:border-white/10">
            <ImageWithFallback 
              src={cv.photoUrl} 
              alt={cv.name || ''} 
