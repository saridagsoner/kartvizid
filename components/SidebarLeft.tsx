    ...
    <div className="flex flex-col gap-5 h-fit">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm transition-all duration-300">
        <div className="p-5 border-b border-gray-100 bg-gray-50/50">
          <h3 className="text-[#1f6d78] font-bold text-sm tracking-tight flex items-center gap-2">
          ...
          {jobFinders.length > 0 ? (
            <div className="space-y-4">
              {jobFinders.slice(0, 5).map((cv) => (
                <div key={cv.id} className="flex items-center gap-3 group cursor-pointer animate-fade-in bg-[#1f6d78] p-1.5 rounded-xl border border-[#1f6d78] hover:bg-white hover:text-black hover:border-gray-200 transition-all">
                  ...
                  <div className="min-w-0">
                    <p className="font-bold text-[11px] truncate leading-tight text-white group-hover:text-black transition-colors">{cv.name}</p>
                    <p className="text-gray-200 text-[9px] truncate group-hover:text-gray-500 transition-colors">{cv.profession}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm transition-all duration-300">
        <div className="p-5 border-b border-gray-100 bg-gray-50/50">
          <h3 className="text-[#1f6d78] font-black text-sm tracking-tight">Talep Gören Meslekler</h3>
        </div>
        <div className="p-5">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm transition-all duration-300">
        <div className="p-5 border-b border-gray-100 bg-gray-50/50">
          <h3 className="text-[#1f6d78] font-black text-sm tracking-tight">Öne Çıkan Şehirler</h3>
        </div>
        <div className="p-5">
          <div className="flex flex-wrap gap-x-3 gap-y-2 transition-all duration-500">
            {(popularCities || []).slice(0, 5).map((city, i) => (
              <span
                key={i}
                className="text-gray-600 text-[12px] font-medium cursor-pointer hover:text-[#1f6d78] transition-colors animate-fade-in"
              >
                {city.label}
              </span>
            ))}
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
        <div className="p-5 border-b border-gray-100 bg-gray-50/50">
          <h3 className="text-[#1f6d78] font-black text-sm tracking-tight">Kartvizid İstatistikleri</h3>
        </div>
        <div className="p-5 space-y-4">
          {platformStats.map((stat, i) => (
            <div key={i} className="flex justify-between items-center text-sm">
              <span className="text-gray-500 font-medium">{stat.label}</span>
              <span className="font-bold text-[#1f6d78]">
                {stat.value}
              </span>
            </div>
          ))}
