...
import { POPULAR_PROFESSIONS, POPULAR_CITIES, EXPERIENCE_LEVELS } from '../constants';
...
const CustomDropdown: React.FC<{
  ...
  return (
    <div className="flex-1 min-w-[130px] relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-2xl border border-gray-100 py-2 z-[100] animate-in slide-in-from-top-2 duration-200">
            ...
            {items.slice(0, 5).map((item, idx) => (
              <button
                ...
                className={`w-full text-left px-5 py-2.5 text-xs font-bold transition-all ${
                  value === item.label ? 'text-black bg-gray-50' : 'text-gray-600 hover:bg-gray-50 hover:text-black'
                }`}
              >
                {item.label}
              </button>
...
const Filters: React.FC<FiltersProps> = ({ currentFilters, onChange }) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-3 shadow-sm flex flex-wrap gap-3 items-center">
      ...
      <button 
        ...
        className={`bg-white border px-5 py-2.5 rounded-full font-bold text-xs transition-all flex items-center gap-2 shadow-sm shrink-0 hover:border-black active:scale-95 ${
          activeFiltersCount > 0 ? 'border-black bg-gray-50' : 'border-gray-200 text-black'
        }`}
      >
